...
public class IfcParserWriterUtils {
    ...
    public static Object convertSimpleValue(PackageMetaData packageMetaData, EStructuralFeature eStructuralFeature, Class<?> instanceClass,
            String value, long lineNumber) throws DeserializeException {
        if (!value.equals("")) {
            if (instanceClass == Integer.class || instanceClass == int.class) {
                ...
                try {
                    ...
                    throw new DeserializeException(DeserializerErrorCode.INVALID_DOUBLE_LITERAL, lineNumber, "Incorrect double floating " +
                            "point value: " + value, e);
                }
                ...
                if (value.startsWith("\"") && value.endsWith("\"")) {
                    ...
                    throw new DeserializeException(DeserializerErrorCode.BYTE_ARRAY_NOT_QUOTED, lineNumber, "Byte[] not starting/ending " +
                            "with \"");
                }
            } else if (IdEObject.class.isAssignableFrom(instanceClass)) {
                throw new DeserializeException(DeserializerErrorCode.UNEXPECTED_TYPE, lineNumber, instanceClass.getSimpleName() + " " +
                        "expected, but got \"" + value + "\"");
            } else {
                throw new DeserializeException(DeserializerErrorCode.UNIMPLEMENTED_BIMSERVER_FEATURE, lineNumber,
                        "Unimplemented " + instanceClass);
            }
        }
        return null;
    }
     ...
     * - The order of parsing (X, X2, X4, S) at the time of writing is arbitrary (and has been changed, the S was in front before,
     * causing problems with IFC files outputted by other software). The ISO docs don't say anything about it.
     ...
     * - This code will process the output of one encoding phase, and possibly use it in the next phase, this is most definitely not
     * according to the standard. Somehow it should be marked which parts are still up for parsing and which parst aren't
            ...
            for (int i = 0; i < stringVal.length(); i++) {
                ...
                if (c == '\'') {
                    ...
                    if (USE_ISO_8859_1) {
                        ...
                        // TODO when multiple sequential characters should be encoded in UCS-2 or UCS-4, we don't really need to add all
                        //  those \X0\ \X2\ and \X4\ chars
                        if (Character.isLowSurrogate(c)) {
                            ...
                            if (i + 1 < stringVal.length()) {
                                ...
                                throw new SerializerException("High surrogate char should be followed by char in low surrogate range, but" +
